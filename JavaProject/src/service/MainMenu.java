...
public class MainMenu {
	private MainDAO ms = MainDAO.getInstance();
	...
	private void allList() {
		...
		ArrayList<UserDTO> a = ms.selectAll();
	...
	private void login() {
		...
		if(ms.select(id).getId().equals(id)) {
			System.out.println("비밀번호 입력");
			String pwd = in.nextLine();
			if(ms.select(id).getPwd().equals(pwd)) {
				if(ms.select(id).getCk().equals("g")) {
